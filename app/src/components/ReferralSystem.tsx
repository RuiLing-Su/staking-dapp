...
const ReferralUser: React.FC<ReferralUserProps> = ({user, isIndirect}) => (
    <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-100">
        <div className="flex items-center justify-between">
            ...
            <span
                className={`px-2 py-1 rounded-full text-xs ${isIndirect ? 'bg-purple-100 text-purple-600' : 'bg-blue-100 text-blue-600'}`}>
        {isIndirect ? '间推' : '直推'}
      </span>
        </div>
    </div>
...
interface ReferralSystemProps {
    referralData?: { direct: any[]; indirect: any[] }
}

const ReferralSystem = ({referralData}: ReferralSystemProps) => {
