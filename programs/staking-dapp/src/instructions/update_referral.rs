...
pub fn update_referral_rewards(
    ...
    // 更新直推人数和团队业绩
    user_info.direct_referrals = user_info.direct_referrals.checked_add(new_referrals).unwrap();
    user_info.team_performance = user_info.team_performance.checked_add(referral_stake_amount).unwrap();
    ...
    pool.global_reward_pool = pool.global_reward_pool.checked_add(referral_stake_amount).unwrap();
